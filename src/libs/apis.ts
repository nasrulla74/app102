...
import { CreateBookingDto, Room, BannerDto, ProductDto } from '@/models/room';
...
export async function getHeroBanner() {
  const result = await sanityClient.fetch<BannerDto>(
    queries.getBannerQuery,
    {},
    { cache: 'no-cache' }
  );
...
export async function getProducts() {
  const result = await sanityClient.fetch<ProductDto>(
    queries.getProducstQuery,
    {},
    { cache: 'no-cache' }
  );
