...
import { send_mail_resend } from '@/app/(web)/actions/sendmail';


const BookingForm = () => {
  const bookingformRef = useRef();
  
  const { onRemove, setShowCart, selectedRoomType, selectedProduct } = useContext(ThemeContext);
  ...
  const sendEmailResend = async (e) => {
    ...
    const mail_data = {
      ...
      arrival_date: startDate.toString().slice(0, 15),
      departure_date: depDate.toString().slice(0, 15),
    }
   
   

    try {
      await send_mail_resend(mail_data);  
